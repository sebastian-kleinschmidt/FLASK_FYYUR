...
def show_venue(venue_id):
  ...
  data.past_shows = db.session.query(Show).join(Artist).filter(Show.venue_id==data.id, Show.start_time<datetime.datetime.now()).all()
  data.upcoming_shows = db.session.query(Show).join(Artist).filter(Show.venue_id==data.id, Show.start_time>datetime.datetime.now()).all()
...
def show_artist(artist_id):
  ...
  data.past_shows = db.session.query(Show).join(Venue).filter(Show.artist_id==data.id, Show.start_time<datetime.datetime.now()).all()
  #data.upcoming_shows = Show.query.filter(Show.artist_id==data.id, Show.start_time>datetime.datetime.now()).all()
  data.upcoming_shows = db.session.query(Show).join(Venue).filter(Show.artist_id==data.id, Show.start_time>datetime.datetime.now()).all()
